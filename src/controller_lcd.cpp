    ...
    while(true) {
        if(control_phase == "opcontrol") {
            if(turret.auto_aim_enabled) {
                if(turret.aiming_for_low_goal) display_on_both_controllers("Auto-Aim: ON | Low", 0);
                else display_on_both_controllers("Auto-Aim: ON | High", 0);
            } else display_on_both_controllers("Auto-Aim: OFF", 0);
            display_on_both_controllers("Target RPM: " + std::to_string((int) turret.launch_RPM), 1);
            display_on_both_controllers("Actual RPM: " + std::to_string((int) (turret.flywheel_mtr_1.getActualVelocity() * ((6.*84.)/64.))), 2);

...
void intake_regulation_function() {
    ...
    while(true) {
        if(intake_enabled) {
            if(((auto_aim_enabled && disk_switch.changedToPressed()) || (!auto_aim_enabled && partner_up.changedToPressed() || auton_indexer_trigger)) && !indexing) {
                indexing = true;
                intake_timer.placeMark();
            }
        ...
        } else if(partner_A.changedToPressed())  turn_roller(alliance_color);
...
bool turn_roller(std::string target_color) {
    ...
    if(target_color == "red") {
        while(roller_optical.get_hue() > 140) intake_mtr.moveVoltage(-12000);
        while(roller_optical.get_hue() < 140) intake_mtr.moveVoltage(12000);
